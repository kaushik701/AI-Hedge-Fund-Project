import sys
from dotenv import load_dotenv
from langchain_core.messages import HumanMessage
from langgraph.graph import StateGraph
from langgraph.graph import END
from colorama import Fore, Style, init
import questionary
import json
import argparse
from datetime import datetime
from dateutil.relativedelta import relativedelta

# Import agents
from agents.ben_graham import ben_graham_agent
from agents.bill_ackman import bill_ackman_agent
from agents.fundamentals import fundamentals_agent
from agents.portfolio_manager import portfolio_management_agent
from agents.technicals import technical_analyst_agent
from agents.risk_manager import risk_management_agent
from agents.sentiment import sentiment_agent
from agents.warren_buffett import warren_buffett_agent
from agents.valuation import valuation_agent

# Import utilities
from graph.state import AgentState
from util.display import print_trading_output
from util.analysts import ANALYST_ORDER, get_analyst_nodes
from util.progress import progress
from llm.models import LLM_ORDER, get_model_info
from util.visualize import save_graph_as_png

# Load environment variables
load_dotenv()
init(autoreset=True)


def parse_hedge_fund_response(response):
    """Parse JSON response from hedge fund agents."""
    try:
        return json.loads(response)
    except json.JSONDecodeError as e:
        print(f"JSON decoding error: {e}")
        return None
    except TypeError as e:
        print(f"Invalid response type: {e}")
        return None
    except Exception as e:
        print(f"Unexpected error: {e}")
        return None


def run_hedge_fund(
    tickers,
    start_date,
    end_date,
    portfolio,
    show_reasoning=False,
    selected_analysts=[],
    model_name="gpt-4o",
    model_provider="OpenAI",
):
    """Run the hedge fund with selected analysts and return trading decisions."""
    # Start progress tracking
    progress.start()

    try:
        # Create workflow if analysts are customized
        if selected_analysts:
            workflow = create_workflow(selected_analysts)
            agent = workflow.compile()
        else:
            agent = app

        # Run the agent with inputs
        final_state = agent.invoke({
            "messages": [
                HumanMessage(content="Make trading decisions based on the provided data.")
            ],
            "data": {
                "tickers": tickers,
                "portfolio": portfolio,
                "start_date": start_date,
                "end_date": end_date,
                "analyst_signals": {},
            },
            "metadata": {
                "show_reasoning": show_reasoning,
                "model_name": model_name,
                "model_provider": model_provider,
            },
        })

        # Return trading decisions and analyst signals
        return {
            "decisions": parse_hedge_fund_response(final_state["messages"][-1].content),
            "analyst_signals": final_state["data"]["analyst_signals"],
        }
    finally:
        # Stop progress tracking
        progress.stop()


def start(state):
    """Initialize the workflow."""
    return state


def create_workflow(selected_analysts=None):
    """Create agent workflow graph with selected analysts."""
    # Create state graph
    workflow = StateGraph(AgentState)
    workflow.add_node("start_node", start)
    
    # Get analyst nodes from configuration
    analyst_nodes = get_analyst_nodes()
    
    # Default to all analysts if none selected
    if selected_analysts is None:
        selected_analysts = list(analyst_nodes.keys())
        
    # Add selected analyst nodes
    for analyst_key in selected_analysts:
        node_name, node_func = analyst_nodes[analyst_key]
        workflow.add_node(node_name, node_func)
        workflow.add_edge("start_node", node_name)
    
    # Add risk and portfolio management nodes
    workflow.add_node("risk_management_agent", risk_management_agent)
    workflow.add_node("portfolio_management_agent", portfolio_management_agent)
    
    # Connect analysts to risk management
    for analyst_key in selected_analysts:
        node_name = analyst_nodes[analyst_key][0]
        workflow.add_edge(node_name, "risk_management_agent")
    
    # Complete the workflow
    workflow.add_edge("risk_management_agent", "portfolio_management_agent")
    workflow.add_edge("portfolio_management_agent", END)
    workflow.set_entry_point("start_node")
    
    return workflow


def initialize_portfolio(tickers, initial_cash, margin_requirement):
    """Initialize portfolio with cash and empty positions."""
    return {
        "cash": initial_cash,
        "margin_requirement": margin_requirement,
        "positions": {
            ticker: {
                "long": 0,
                "short": 0,
                "long_cost_basis": 0.0,
                "short_cost_basis": 0.0,
            } for ticker in tickers
        },
        "realized_gains": {
            ticker: {
                "long": 0.0,
                "short": 0.0,
            } for ticker in tickers
        }
    }


def select_analysts():
    """Prompt user to select analysts."""
    choices = questionary.checkbox(
        "Select your AI analysts.",
        choices=[questionary.Choice(display, value=value) for display, value in ANALYST_ORDER],
        instruction="\n\nInstructions: \n1. Press Space to select/unselect analysts."
                "\n2. Press 'a' to select/unselect all."
                "\n3. Press Enter when done to run the hedge fund.\n",
        validate=lambda x: len(x) > 0 or "You must select at least one analyst.",
        style=questionary.Style([
            ("checkbox-selected", "fg:green"),
            ("selected", "fg:green noinherit"),
            ("highlighted", "noinherit"),
            ("pointer", "noinherit"),
        ]),
    ).ask()

    if not choices:
        print("\n\nInterrupt received. Exiting...")
        sys.exit(0)
        
    print(f"\nSelected analysts: "
        f"{', '.join(Fore.GREEN + choice.title().replace('_', ' ') + Style.RESET_ALL for choice in choices)}\n")
    
    return choices


def select_model():
    """Prompt user to select LLM model."""
    model_choice = questionary.select(
        "Select your LLM model:",
        choices=[questionary.Choice(display, value=value) for display, value, _ in LLM_ORDER],
        style=questionary.Style([
            ("selected", "fg:green bold"),
            ("pointer", "fg:green bold"),
            ("highlighted", "fg:green"),
            ("answer", "fg:green bold"),
        ])
    ).ask()

    if not model_choice:
        print("\n\nInterrupt received. Exiting...")
        sys.exit(0)
        
    model_info = get_model_info(model_choice)
    if model_info:
        model_provider = model_info.provider.value
        print(f"\nSelected {Fore.CYAN}{model_provider}{Style.RESET_ALL} model: "
            f"{Fore.GREEN + Style.BRIGHT}{model_choice}{Style.RESET_ALL}\n")
    else:
        model_provider = "Unknown"
        print(f"\nSelected model: {Fore.GREEN + Style.BRIGHT}{model_choice}{Style.RESET_ALL}\n")
        
    return model_choice, model_provider


def validate_date(date_str, format="%Y-%m-%d"):
    """Validate date string format."""
    if not date_str:
        return None
        
    try:
        return datetime.strptime(date_str, format).strftime(format)
    except ValueError:
        raise ValueError(f"Date must be in {format} format")


def get_date_range(start_date, end_date):
    """Get validated start and end dates."""
    # Validate end date
    end_date = validate_date(end_date) or datetime.now().strftime("%Y-%m-%d")
    
    # Calculate start date if not provided
    if not start_date:
        end_date_obj = datetime.strptime(end_date, "%Y-%m-%d")
        start_date = (end_date_obj - relativedelta(months=3)).strftime("%Y-%m-%d")
    else:
        start_date = validate_date(start_date)
        
    return start_date, end_date


if __name__ == "__main__":
    # Parse command line arguments
    parser = argparse.ArgumentParser(description="Run the hedge fund trading system")
    parser.add_argument("--initial-cash", type=float, default=100000.0,
                        help="Initial cash position (default: 100000.0)")
    parser.add_argument("--margin-requirement", type=float, default=0.0,
                        help="Initial margin requirement (default: 0.0)")
    parser.add_argument("--tickers", type=str, required=True,
                        help="Comma-separated list of stock ticker symbols")
    parser.add_argument("--start-date", type=str,
                        help="Start date (YYYY-MM-DD, default: 3 months before end date)")
    parser.add_argument("--end-date", type=str,
                        help="End date (YYYY-MM-DD, default: today)")
    parser.add_argument("--show-reasoning", action="store_true",
                        help="Show reasoning from each agent")
    parser.add_argument("--show-agent-graph", action="store_true",
                        help="Show the agent graph")

    args = parser.parse_args()

    # Parse tickers
    tickers = [ticker.strip() for ticker in args.tickers.split(",")]
    
    # Select analysts and model
    selected_analysts = select_analysts()
    model_choice, model_provider = select_model()
    
    # Create workflow
    workflow = create_workflow(selected_analysts)
    app = workflow.compile()
    
    # Show agent graph if requested
    if args.show_agent_graph:
        graph_filename = "_".join(selected_analysts) + "_graph.png"
        save_graph_as_png(app, graph_filename)
    
    # Get date range
    start_date, end_date = get_date_range(args.start_date, args.end_date)
    
    # Initialize portfolio
    portfolio = initialize_portfolio(tickers, args.initial_cash, args.margin_requirement)
    
    # Run hedge fund
    result = run_hedge_fund(
        tickers=tickers,
        start_date=start_date,
        end_date=end_date,
        portfolio=portfolio,
        show_reasoning=args.show_reasoning,
        selected_analysts=selected_analysts,
        model_name=model_choice,
        model_provider=model_provider,
    )
    
    # Print results
    print_trading_output(result)